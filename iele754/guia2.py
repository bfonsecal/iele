...
#Cuantas comunas hay en chile
cols= ['Comuna']
for col in cols:
  print(f'Columna {col}: {df_tidy[col].nunique()} subniveles')
  

df_arica = df_tidy.loc[df_tidy['Comuna'] == 'Arica']



df_arica['value'] = df_arica['value'].astype(float)
df_arica['Poblacion'] = df_arica['Poblacion'].astype(float)

# calcular la incidencia para Arica
df_arica['incidencia'] = df_arica['value']/df_arica['Poblacion']
print(df_arica['incidencia'].mean()*100)
